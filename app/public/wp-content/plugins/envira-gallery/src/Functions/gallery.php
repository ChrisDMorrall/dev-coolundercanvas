...
function envira_get_gallery_config( $gallery_id, $raw = false, $data = null ){
	...
	if ( !empty( $data ) && $data['config']['type'] == 'dynamic' ){
		...
		$data = _envira_get_gallery( $gallery_id );

		// temp hack: preserve keyboard and mousewheel settings (see 1980)

		$keyboard   = isset( $data['config']['keyboard'] ) ? $data['config']['keyboard'] : 0;
		$mousewheel = isset( $data['config']['mousewheel'] ) ? $data['config']['mousewheel'] : 0;
...
function envira_get_gallery_images( $gallery_id, $raw = false, $data = null, $return_sort_ids = false, $for_albums = false ){
	...
	$setting =  isset( $data['config']['pagination'] ) ? $data['config']['pagination'] : false;
	...
	if ( isset( $data['gallery'] ) && is_array( $data['gallery'] ) ){
		...
		foreach( (array) $data['gallery'] as $id => $item ) {
			...
			$item['alt'] = envira_santitize_title( $item['alt'] );

			$item['gallery_id'] = $gallery_id;

			/* album specific info */
			if ( $for_albums ) {
				$item['gallery_title'] = envira_get_config( 'title', $data ) ? envira_get_config( 'title', $data ) : get_the_title( $gallery_id ) ;
			}
	...
	$full_data = array ( 'gallery_images' => json_encode( $images, JSON_UNESCAPED_UNICODE ), 'sorted_ids' => json_encode( $id_array, JSON_UNESCAPED_UNICODE ) );
